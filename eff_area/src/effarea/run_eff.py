...
if __name__ == "__main__":
    ...
    swift_grb = check_swift(GRB, grb_time)
    ...
    def get_bb_edges(start_times, mask, count_sum):
        ...
        for e, edge in enumerate(bb_edges):
            if e < len(bb_edges) - 1:
                ...
                stop_temp = start_times[
                    np.argwhere(start_times <= bb_edges[e + 1])[-1, 0]
                ]
                if start_temp == stop_temp:
                    stop_temp = start_times[
                        np.argwhere(start_times == start_temp)[-1, 0] + 1
                    ]
    ...
    results_yml = os.path.join(
        os.environ.get("GBMDATA"), f"localizing/trigdat_norms.yml"
    )
    final_dict = {}
    ...
    for s in range(len(bb_edges_start)):
        ...
        if selection in dir_list:
            ...
            bayes.results.write_to(
                result_path + "/grb_" + trigger + "_" + spectrum + ".fits",
                overwrite=True,
            )
            ...
            if rank == 0:
                ...
                result_dict = {}
                result_dict["K"] = float(
                    opt_model["grb.spectrum.main.Cutoff_powerlaw.K"].value
                )
                result_dict["index"] = float(
                    opt_model["grb.spectrum.main.Cutoff_powerlaw.index"].value
                )
                result_dict["xc"] = float(
                    opt_model["grb.spectrum.main.Cutoff_powerlaw.xc"].value
                )
                for d in lu:
